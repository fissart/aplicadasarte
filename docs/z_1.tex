...
\setcounter{enumi}{1}
\item
  Toque. Existe relación de toque cuando las formas están en contacto \(F_1\cap F_2={p}\) (conjunto unitario).
\item
  Superposición. Existe superposición cuando una forma tapa a otra y de esta solo vemos la parte que no está tapada \(F_1\cup (F_2-F_1)\).
\item
  Unión. Dos formas tienen una relación de unión cuando se funde y los contornos de las dos formas se complementan \(F_2\cup F_1\).
\item
  Intersección. Existe relación de intersección entre dos formas cuando surge una nueva forma fruto de la zona común entre ellas. Es decir, ambas formas comparten una superficie del plano y esta genera una nueva forma \(F_2\cap F_1\).
\item
  Sustracción. La relación de sustracción se produce cuando una forma no aparece representada, pero resta el espacio común a la otra creando un vacío \(F_2-F_1\).
\item
  Penetración. Existe relación de penetración entre formas cuando se crea un vacío en el espacio común entre ellas \(F_2\: \Delta\: F_1\).
\item
  Transparencia. En una relación de transparencia entre formas, ambas formas comparten un espacio en común, pero se aprecia el contorno de ambas. Podemos entenderlo como una superposición de una forma translúcida sobre otra (\(F_2\cup F_1)\cup (F_1\cap F_2\)).
\end{enumerate}
...
\begin{definition}[Transformaciones]
\protect\hypertarget{def:transformaciones}{}\label{def:transformaciones}Es un conjunto de procesos que se realiza a alguna forma en particular sin modificar la forma.
\end{definition}
...
\begin{definition}[Transformaciones topológicas]
\protect\hypertarget{def:transformacionestopo}{}\label{def:transformacionestopo}Consiste en doblar, estirar, encoger, retorcer, etc., los objetos, pero siempre que se haga sin romper ni separar lo que estaba unido, ni pegar lo que estaba separado. Por ejemplo, un triángulo es topológicamente lo mismo que una circunferencia, ya que
...
\begin{example}[Transformación topológicas de la $N$ esfera]
...
\url{https://solitaryroad.com/c775.html}
